...
export class McpAuthJwtGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const configured = (this.oauthOptions.resource || '')
      .split(/[,\s]+/)
      .filter(Boolean);
    const envList = (process.env.OAUTH_ALLOWED_RESOURCES || '')
      .split(/[,\s]+/)
      .filter(Boolean);
    const allowed = (
      configured.length ? configured : [this.oauthOptions.resource]
    )
      .concat(envList)
      .filter(Boolean);
    ...
    const matchAny = (val: string | undefined, list: string[]) =>
      !!val && list.some((r) => r === val);
    const audOk = Array.isArray(aud)
      ? aud.some((a) => matchAny(a, allowed))
      : matchAny(aud, allowed);
    const resOk = matchAny(resClaim, allowed);

    if (!(audOk && resOk)) {
      throw new UnauthorizedException(
        'Invalid audience/resource for this MCP server',
      );
    }
